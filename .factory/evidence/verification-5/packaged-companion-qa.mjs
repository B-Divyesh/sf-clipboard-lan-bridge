...
const result = {
  status: response?.status(), semantic,
  axeSeriousCritical: severe.map(item => ({ id: item.id, impact: item.impact })),
  invalidMessage, code, codeLength: code?.length, storage,
  requestOrigins: [...new Set(requests.map(url => new URL(url).origin))],
  consoleErrors, pageErrors,
};
console.log(JSON.stringify(result, null, 2));
await page.screenshot({ path: ".factory/evidence/verification-5/packaged-companion-mobile.png", fullPage: true });
await context.close();
await browser.close();
